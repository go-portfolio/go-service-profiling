# Go Profiling Service 🚀

Сервис для демонстрации профилирования CPU, памяти и задержек в Go, с интеграцией метрик Prometheus и поддержкой Docker.

---

## 🚀 Быстрый старт

### 1. Клонируйте репозиторий

```bash
git clone <your-repo-url>
cd go-service-profiling
```
2. Установите зависимости
```bash
go mod tidy
```
3. Запуск сервиса
```bash
make run
```
Сервис будет доступен на http://localhost:8080

## ⚙️ Доступные эндпоинты
| URL             | Описание                                                |
| --------------- | ------------------------------------------------------- |
| `/`             | Корневая страница, сообщает что сервис запущен          |
| `/work`         | Имитирует нагрузку CPU                                  |
| `/alloc`        | Имитирует выделение памяти и сборку мусора              |
| `/sleep`        | Имитирует задержку (sleep)                              |
| `/metrics`      | Экспорт метрик Prometheus                               |
| `/debug/pprof/` | Профилирование через pprof (CPU, heap, goroutine и др.) |


## 📊 Профилирование
CPU профайл
```bash
./scripts/profile-cpu.sh
# Сохранится cpu.prof
# Далее откроем веб-интерфейс:
go tool pprof -http=:8081 ./bin/service cpu.prof
```
Heap профайл (память)
```bash
./scripts/profile-heap.sh
# Сохранится heap.prof
# Веб-интерфейс:
go tool pprof -http=:8081 ./bin/service heap.prof
```
Веб-интерфейс pprof позволяет интерактивно изучать, какие функции потребляют CPU и память.

##  Бенчмарки
Запуск всех бенчмарков с измерением памяти:

bash
```
make bench
```
-bench=. — запуск всех бенчмарков

-benchmem — измерение использования памяти

## 🐳 Docker
Сборка Docker-образа:

```bash
make docker-build
```
Запуск контейнера:

```bash
docker run -p 8080:8080 go-service-profiling:latest
```
## 📈 Метрики Prometheus
Сервис экспортирует метрики на /metrics.
Пример метрики:

```nginx
http_requests_total 42
```
`http_requests_total` — общее количество HTTP-запросов к сервису


## Структура проекта
```bash
go-service-profiling/
├── cmd/server/          # main.go — точка входа
├── internal/
│   ├── handlers/        # HTTP-хендлеры
│   ├── metrics/         # метрики Prometheus
│   └── workload/        # функции имитации нагрузки (CPU, memory, sleep)
├── tests/               # тесты и бенчмарки
├── scripts/             # скрипты для профилирования (CPU, heap и др.)
├── bin/                 # собранный бинарник
├── Makefile             # сборка, запуск, бенчмарки, Docker
└── README.md
```
💡 Советы
- Используйте `/work`, `/alloc` и `/sleep` для генерации нагрузки перед профилированием.
- Открывайте профили через `go tool pprof -http=:8081`, чтобы видеть графики и горячие функции.
- Метрики `Prometheus` можно подключать к `Grafana` для визуализации.

## Goroutine профайл

```bash
./scripts/profile-goroutine.sh
# Сохранится goroutine.prof
# Далее откроем веб-интерфейс:
go tool pprof -http=:8081 ./bin/service goroutine.prof


## Снятие полезного CPU-профиля

Чтобы получить информативный CPU-профиль в проекте `go-service-profiling`, нужно создать реальную нагрузку на процессор во время профилирования.  

### Шаги

1. **Запустить сервис**

```bash
./bin/service


2. Создать нагрузку на CPU (в другом терминале) одновременно с шагом 3:
```bash
curl http://localhost:8080/work
# или несколько раз
for i in {1..5}; do curl http://localhost:8080/work; done
```
Это вызовет хендлер WorkHandler, который запускает функцию CPUHeavy для имитации нагрузки.

3. Снять CPU-профиль одновременно с шагом 2:
```bash
curl "http://localhost:8080/debug/pprof/profile?seconds=30" -o cpu.prof
```
Или через make-скрипт:
```bash
make cpu-prof
```
4. Открыть профиль для анализа
```bash
go tool pprof -http=:8081 ./bin/service cpu.prof
```
Откроется веб-интерфейс по адресу http://localhost:8081.

В нём можно посмотреть:
Flamegraph — функции, где тратится больше всего CPU.
Call graph — как функции вызывают друг друга.
Top — список «горячих» функций с временем выполнения.
Source view — время по строкам исходного кода.

⚠️ Важно: без вызова /work профиль будет пустым (Total samples = 0), потому что CPU не использовался.


## Снятие Block-профиля

Block profile позволяет определить участки кода, где горутины блокируются, ожидая доступа к мьютексам, каналам или другим синхронизирующим структурам.

### Шаги

Запустить сервис
```block
./bin/service
```

Создать конкурентную нагрузку на блокировки (в другом терминале), например через хендлер WorkHandler:
```bash
curl http://localhost:8080/work
# или несколько раз
for i in {1..5}; do curl http://localhost:8080/work; done
```

Эти вызовы создадут точки с высокой конкуренцией потоков.

Снять block-профиль:
```bash
curl "http://localhost:8080/debug/pprof/block?seconds=30" -o block.prof
```

Или через make-скрипт:
```bash
make block-prof
```

Открыть профиль для анализа:
```bash
go tool pprof -http=:8081 ./bin/service block.prof
```

Откроется веб-интерфейс по адресу http://localhost:8081
.

В нём можно посмотреть:

`Top` — функции, где горутины дольше всего ждали блокировки.

`Flamegraph` — визуализация блокирующих вызовов.

`Call graph` — как функции вызывают друг друга при блокировках.

⚠️ Важно: без нагрузки на синхронизирующие объекты (`/work` или другие конкурентные операции) профиль будет пустым, потому что блокировок не было.


## 🔹 Профилирование многопоточности (ThreadCreate и Mutex)

**Что это:**  
- **ThreadCreate** — собирает информацию о создании новых потоков (горутин).  
- **Mutex** — собирает данные о конкуренции за мьютексы, где горутины ждут блокировку.  

Для полезного результата профиль нужно снимать одновременно с нагрузкой:
- Mutex — моментальный снимок конкуренции за мьютексы.
- ThreadCreate — моментальный снимок созданных горутин.

**Зачем нужно:**  
- Помогает выявлять узкие места с высокой конкуренцией.  
- Позволяет оптимизировать синхронизацию и уменьшить задержки в многопоточных участках кода.

**Как использовать:**
⚠️ Если снимать без нагрузки, файлы будут почти пустыми и малоинформативными:
```bash
# Создаём нагрузку
for i in {1..5}; do curl http://localhost:8080/work & done
```

1. ThreadCreate профайл:

```bash
curl "http://localhost:8080/debug/threadcreate" -o threadcreate.prof
go tool pprof -http=:8081 ./bin/service threadcreate.prof
```

2. Mutex профайл:
```bash
curl "http://localhost:8080/debug/mutex" -o mutex.prof
go tool pprof -http=:8081 ./bin/service mutex.prof
```

| Профиль      | Что измеряет                   | Время ожидания | Конкуренция | Создание потоков |
| ------------ | ------------------------------ | -------------- | ----------- | ---------------- |
| Block        | Все блокировки (mutex, канал)  | Да             | Да          | Нет              |
| Mutex        | Только мьютексы                | Да             | Да          | Нет              |
| ThreadCreate | Создание новых горутин/потоков | Нет            | Нет         | Да               |
